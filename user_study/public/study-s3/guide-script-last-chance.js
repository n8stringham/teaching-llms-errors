...
function writeUserData() {
  ...
  db.collection("responses_mmlu" + onboarding_setting)
    .doc(response_id)
    .update({
      time_spent_guidelines2nd: timeSpent,
      guidelines2nd_clicks: clickCounts,
    //   guidelines2nd_names: guidelineNames,
      guidelines2nd_click_seq: clickSeq,
      guidelines2nd_time_seq: timeSeq,
      example_counts2nd: exampleCounts,
    })
    .then(() => {
      console.log("Response successfully written!");
      nextPage('./test-phase.html');
    })
    .catch((error) => {
