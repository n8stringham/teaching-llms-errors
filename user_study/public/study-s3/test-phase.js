...
function getQuestions() {
    ...
    return new Promise((resolve, reject) => {
        ...
        db.collection("tasks_mmlu_cleaned_unmatchedQs")
            ...
            // const questions_no_guideline = data[task_id].questions_no_guideline.slice(-5)
            ...
            const initTopics = combineLists(Array(questions_no_guideline.length).fill(""), data[task_id].subjects_noai)
            ...
            globalQs = questions
            globalTopics = shuffleArray(47, initTopics)
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelector("#followUpButton").addEventListener("click", function () {
        ...
        // Minimum length restriction (e.g., 10 characters)
        const minLength = 50;
        console.log('logic', (selectedValues['used-guideline'] != "no-guideline" && selectedValues['used-guideline'] != "forgot"))
        if (freeResponseValue.length < minLength && (selectedValues['used-guideline'] != "no-guideline" && selectedValues['used-guideline'] != "forgot")) {
            alert(`Response must be at least ${minLength} characters long.`);
            return; // Stop execution if the requirement isn't met
        }
