...
function getQuestions() {
    ...
    return new Promise((resolve, reject) => {
        ...
        db.collection("tasks_mmlu_cleaned_unmatchedQs")
            ...
            const questions_no_guideline = data[task_id].questions_no_guideline.slice(0,3)
            const initQuestions = combineLists(questions_no_guideline, data[task_id].teaching_ours_images)
            ...
            const initAiAnswerLetter= combineLists(Array(questions_no_guideline.length).fill(-1), data[task_id].teaching_ours_ai_answers_raw);
            
            const initGoldLabel = combineLists(Array(questions_no_guideline.length).fill(-1), data[task_id].teaching_ours_labels);
            ...
            const initCorrectRely = combineLists(Array(questions_no_guideline.length).fill(""), data[task_id].teaching_ours_recs);
...
function writeUserData() {
    ...
    db.collection("responses_mmlu" + onboarding_setting)
      .doc(response_id)
      .update({ 
        practice_user_answers_str: personErrorAnswers,
        practice_user_answers_int: personErrorAnswersInt,
        practice_user_answers_action: personActionAnswers,
        practice_used_guideline: usedGuidelines,
        practice_free_response: freeResponseExplanation,
        // array which records the recommended behavior ignore/rely/unsure ai per question
        practice_recs : correctRely, 
        // mapped to ints 0, 1, 2
        practice_recs_int: correctRelyInt,
        // array which records whether ai was correct or not
        practice_ai_correct: aiCorrect,
        practice_time_per_q: questionTimes,
        practice_qs: globalQs
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelector("#followUpButton").addEventListener("click", function () {
        ...
        function generateFeedback(userCorrect, userX, aiX) {
            if (userCorrect === 1 && cLabel !== 2) { //  I would/wouldn't matches the recommendation
            ...
            } else if (userCorrect === 1 && cLabel === 2 && selectedValues['used-guideline'] === "no-guideline") { // uncertain and there is no recommendation
                return `You were Correct! <br> You answered "${userX}" and the example above does not match any of the guidelines.`;
            } else if (userCorrect === 1 && cLabel === 2 && selectedValues['used-guideline'] === "partial-guideline") {
                return `Hmm...We deem that none of the guidelines match this question, but there is often room for interpretation in how to apply them. We will review the guideline(s) you suggested.`;
            } else if (userCorrect === 1 && cLabel === 2 && selectedValues['used-guideline'] === "forgot") {
                return `That's okay! It can be a lot to remember. It turns out this question doesn't match any of the guidelines. You will have a chance to review the full guidelines again later.`;
            } else if (userCorrect === 0 && selectedValues['used-guideline'] === "forgot") {
                return `That's okay! It can be a lot to remember. It turns out the example above is similar to questions where AI is typically ${aiX}. Please review the details below. You will have a chance to review the full guidelines again later.`;
            } else if (userCorrect === 0 && cLabel === 2 ) {
                return `Hmm...We deem that none of the guidelines match this question, so you should actually be uncertain about whether to use AI to get an answer.`;
            } else {
...
function writeTimeSpent() {
  ...
  db.collection("responses_mmlu" + onboarding_setting)
    ...
    //   window.location.href = "test-phase.html"; // Go to End Page
      window.location.href = "guide-last-chance.html"; // Go to End Page
    })
    .catch((error) => {
      console.error("Error writing response: ", error);
    });
