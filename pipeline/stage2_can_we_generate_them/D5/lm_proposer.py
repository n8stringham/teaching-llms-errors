...
rm_cmp_prompt = open('stage2_can_we_generate_them/D5/templates/rm_cmp_prompt.txt').read()
...
class GPT3_Proposer:
    ...
    def propose_hypotheses(self, X_A: List[str], X_B: List[str]):
        ...
        arg_dict = {k: self.problem[k] for k in ['dataset_description', 'generation', 'A_desc', 'B_desc', 'user', 'target']}
        ...
        while num_incontext_samples > 1:
            ...
            if prompt_length < MAX_PROMPT_LENGTH:
                ...
                print(f"Prompt too long, reducing context samples to {num_incontext_samples - 1}")
                num_incontext_samples -= 1

        arg_dict['A_block'] = sent_subset['A_block']
        arg_dict['B_block'] = sent_subset['B_block']
        final_prompt = self.prompt_template.format(**arg_dict)
        ...
        hs = []
        for h in returned_text.split('\n-'):  # Adjust to handle different formats better
            # h = convert_cmp_to_ind(h.replace('"', '').strip())
            if h:  # Ensure non-empty hypothesis
                if h.endswith('.'):
                    h = h[:-1]
                hs.append(h)
