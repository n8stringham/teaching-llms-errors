...
def query_openai(prompt, model="gpt-4o-2024-08-06", t=TEMPERATURE):
    ...
    # Extract top logprobs for the returned token
    top_logprobs = response.choices[0].logprobs.content[0].top_logprobs[0]
    answer_text = response.choices[0].message.content
    score = math.exp(top_logprobs.logprob)
    # print('top_logprobs=', top_logprobs)
    if answer_text.lower() == 'no':
        score = 1 - score

    # Convert logprob to probability if found
...
class OpenAIValidator:
    ...
    def __init__(self, template: str = DEFAULT_VALIDATOR_TEMPLATE):
